...
- **Allenamento**: **A1** timer/cronometro sessione · **A2** ✅ risultati per esercizio (kg/rip) + "l'ultima volta" + storico sedute — fatto con il rework S3/S4/S5, vedi `docs/REWORK-ALLENAMENTO.md`; le spunte `localStorage` non esistono più; **S6** ✅ il consiglio di Keiko (`lib/coach.ts`: volo domani → seduta corta, +2,5 kg se l'ultima volta era piena); resta il **grafico progressi** · **A3** immagini esercizi affidabili (ExerciseDB) — ora fallback generico.
